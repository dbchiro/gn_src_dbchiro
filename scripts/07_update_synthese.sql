...
DROP FUNCTION IF EXISTS fct_tri_c_upsert_dbchiro_observation_to_geonature() CASCADE
;

CREATE OR REPLACE FUNCTION fct_tri_c_upsert_dbchiro_observation_to_geonature() RETURNS TRIGGER
    LANGUAGE plpgsql AS
$$
DECLARE
    the_record      RECORD;
...
BEGIN
    SELECT r.*
    INTO the_record
    FROM (WITH obs AS (SELECT ss.*
                            , taxref.lb_nom
                       FROM sights_sighting ss
                                JOIN dicts_specie ds ON ss.codesp_id = ds.id
                                JOIN cor_sp_dbchiro_taxref cor_tx ON cor_tx.idsp_dbchiro = ds.id
...
--                   , gn_synthese.get_default_nomenclature_value('TECHNIQUE_OBS')   AS id_nomenclature_obs_technique
              ...
              -- TODO: confirmer les choix
                        ...
                        , CASE
                              WHEN total_count = 0 THEN ref_nomenclatures.get_id_nomenclature('STATUT_OBS', 'No')
              ...
              /*               , CASE
               ...
               END                                                                 AS observers*/
                  ...
                  coalesce(
                        ...
                        , replace('https://gcra.dbchiro.org/sighting/{ID_SIGHTING}/detail', '{ID_SIGHTING}',
                                  obs.id_sighting::TEXT)                                         AS reference_biblio
          FROM obs
                   JOIN dicts_specie ds ON obs.codesp_id = ds.id
                   JOIN sights_session ses
                        ON obs.session_id = ses.id_session --                          LEFT JOIN sobs ON sobs.id_sighting = obs.id_sighting
                   JOIN accounts_profile a ON ses.main_observer_id = a.id
                   LEFT JOIN sights_place spl ON ses.place_id = spl.id_place
                   LEFT JOIN dicts_typeplace typeplace ON spl.type_id = typeplace.id
                   LEFT JOIN dicts_placeprecision dplp ON spl.precision_id = dplp.id
                   LEFT JOIN dicts_contact c2 ON ses.contact_id = c2.id
                   LEFT JOIN geodata_municipality m2 ON spl.municipality_id = m2.id
                   LEFT JOIN sights_countdetail scd ON obs.id_sighting = scd.sighting_id
                   LEFT JOIN cor_sp_dbchiro_taxref cor_tx ON cor_tx.idsp_dbchiro = ds.id
                   LEFT JOIN management_study mst ON ses.study_id = mst.id_study
             , (SELECT id_source
                FROM gn_synthese.t_sources
                WHERE name_source LIKE 'dbChiroGCRA') AS source) AS r;
    IF found THEN
        ...
        VALUES ( the_id_synthese
               ...
               , the_record.place
               , the_record.is_valid)
        ON CONFLICT (id_synthese) DO UPDATE
            SET id_sp_source        = the_record.id_sp_source
...
ALTER FUNCTION fct_tri_c_upsert_dbchiro_observation_to_geonature() OWNER TO geonature
;
...
DROP TRIGGER IF EXISTS fct_tri_c_upsert_dbchiro_observation_to_geonature ON sights_sighting
;
...
DROP TRIGGER IF EXISTS tri_update_observatio_on_place_update ON sights_place;
;

CREATE TRIGGER tri_update_observatio_on_place_update
...
CREATE FUNCTION fct_tri_update_observation_on_session_update() RETURNS TRIGGER AS
...
DROP TRIGGER IF EXISTS tri_update_observation_on_session_update ON sights_session;
...
ALTER FUNCTION fct_tri_c_delete_dbchiro_observation_from_geonature() OWNER TO geonature
;
...
DROP TRIGGER IF EXISTS tri_c_delete_dbchiro_observation_from_geonature ON sights_sighting
;
;
